...
- **Edição na própria barra (handles arrastáveis):** no modo edição, os limites entre segmentos da faixa de composição viram handles arrastáveis. Arrastar redistribui o alvo entre as duas classes adjacentes, **mantendo a soma em 100% por construção** (mexer numa fronteira só transfere pontos entre vizinhos). Implementar como handles custom com `role="slider"`, `aria-valuenow/min/max`, `tabindex=0` e setas do teclado (passo 1pp, Shift+seta 5pp); pointer events para o arraste (`pointerdown/move/up`), com fallback de toque. O input numérico de alvo sai; o **limite de concentração** segue como input pequeno.

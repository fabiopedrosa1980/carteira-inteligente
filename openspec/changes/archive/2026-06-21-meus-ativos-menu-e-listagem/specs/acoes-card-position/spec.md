...
### Requirement: Card de Meus Ativos exibe quantidade, preço atual, saldo, variação e rentabilidade

O card de ativo em "Meus Ativos" SHALL exibir, por ativo: a **quantidade** de cotas, o **preço atual** (cotação do dia), o **saldo atual** da posição, a **variação da posição em R$** e a **rentabilidade em %**. Os valores derivados MUST ser calculados no frontend a partir de `quantity`, `avg_price` (preço médio) e `current_price` (preço atual):
...
A variação do dia ("Hoje", `change_percent`) **NÃO** é exibida no card. Variação (R$) e rentabilidade (%) MUST indicar sinal e cor (positivo/negativo).
...
- **AND** Hoje (change_percent) não é exibido no card

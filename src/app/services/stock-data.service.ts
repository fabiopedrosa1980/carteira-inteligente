...
export class StockDataService {
  ...
  private readonly _portfolioRefs = signal<{ id: number; ticker: string; historyReady: boolean }[]>(
    [],
  );
  ...
  /** Recarrega a carteira agregada a partir do backend (ex.: após alterar lançamentos). */
  reload(): void {
    this.fetchPortfolioStocks();
  }
    ...
    this.api
      ...
      .subscribe({
        next: ({ apiStocks, allDividends }) => {
          this._portfolioRefs.set(
            apiStocks.map((s) => ({
              id: s.id,
              ticker: s.ticker,
              historyReady: s.history_ready ?? false,
            })),
          );
          this._stocks.set(
            apiStocks.map((s, i) => ({
  ...
  getDividendsForMonth(year: number, month: number): { stock: Stock; dividend: DividendRecord }[] {
    return this._stocks()
      .map((stock) => ({
        stock,
        dividend: stock.dividends.find((d) => d.year === year && d.month === month),
      }))

...
Define o endpoint `GET /api/v1/transactions/etfs` que retorna as posições ETF agregadas do usuário, enriquecidas com cotação em tempo real e nota calculada, no mesmo formato dos endpoints `/acoes` e `/fiis`.
...
- **WHEN** retornam 3 ou mais posições ETF com preços válidos
